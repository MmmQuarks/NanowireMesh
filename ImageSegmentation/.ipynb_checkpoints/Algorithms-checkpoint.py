...
def radon_angle(image, theta=None):
	"""
	...
	for angle in tqdm(np.deg2rad(theta)):
...
def spliner(coords):
	...
	if len(coords) <= 10:
		...
		k = 3
		num_anchors = 4 * np.floor(len(coords) / 20 + 1)	# we want 4 anchor points for every 20 pixels
...
def labeled_img_to_graph(img, add_electrodes = True):
	'''
	args
		img
			labeled and skeletonized image of segments
		add_electrodes
	...
	ordered_segment_pixels = dict()
	pbar = tqdm(segment_labels)
	pbar.set_description('Ordering Segment Pixels')
	for segment in pbar:
		try:
			rag = segment_rag(img, segment)
			rag = reconnect_segment_rag(rag)
			path = diameter_path(rag) #nx.shortest_path(rag, source = tip1, target = tip2)
			idx_list = [rag.nodes[node]['idx'] for node in path]
			ordered_segment_pixels[segment] = idx_list
		except Exception as e:
			print(e)
			# raise Exception(
				# 'Problem on segment {}: {}'.format(segment, e)
			# )
	...
	for segment, ordered_pixels in ordered_segment_pixels.items():
		try:
			...
			u, tck = spliner(ordered_coords)
			x,y = splev(u,tck)[:2] # first two elements returned by splev are x,y
			shape = LineString([(x[n], y[n]) for n in range(len(x))])
			g.add_node(
				segment,
				idx = ordered_pixels,
				spline = (u,tck),
		...
		try:
			...
			if X.is_empty:
				...
				if type(X) == Point:
					...
					raise Exception('Intersection of type {} but must be Point of MultiPoint'.format(type(X)))
		except Exception as e:
