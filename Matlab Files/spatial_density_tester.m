function [network point] = spatial_density_tester(Length,width, NW_length, percolationMultiple)

    Length = 200;
    width = 200;
    NW_length = 10;
    percolationMultiple = 3;
...
%         originY = Length * rand(step_size,1); % start of nanowire y position
        % new version for generating nonuniform spatial density
        %(decides whether will be on dense or not dense square)
        if randi([1,64]) <= 52 % dense square
            squareNum = randi([1,13]);
            
            %coordinates of bottom left corner of each square
            squareCoords = [0 0;
                            .4 0;
                            .8 0;
                            .2 .2;
           ...
           squareNum = randi([1,12]);
           
           %coordinates of bottom left corner of these squares
           squareCoords = [.2 0;
                            .6 0;
                            0 .2;
                            .4 .2;
        ...
        originX = (squareCoords(squareNum, 1) + 0.2 * rand()) * width;
        originY = (squareCoords(squareNum, 2) + 0.2 * rand()) * Length;
