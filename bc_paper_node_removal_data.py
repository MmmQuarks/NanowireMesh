...
removalMethodsToSkip = [m for m in removalMethodsList if 'DownPotential' in m or 'high' in m]
removalMethodsToCalculate = set(removalMethodsList) - set(removalMethodsToSkip)
...
shouldCalculateInitialCentralities = False
shouldMakePlot = True
shouldMakeAverageFailurePlot = True
shouldNormalizeResistances = True

#note: if testing, numSamples must be >= 2 because we calculat std at the end
numSamples = 10


# making a netlist suffix so different instances of this script can run concurrently
netlistSuffix = '_'.join(sys.argv[1:])

# making easy way to change path to xyce in code
#xycePath = '/home/amwt/TPV/2DNanowires/installations/build_Xyce-6.11_parallel_openMPI/bin/Xyce'
xycePath = 'xyce'

# name directory where existing networks can be found
existingNetworkFolder = '/home/amwt/TPV/2DNanowires/Data/evolution_under_node_removal/'

# results folder name - used in saving and naming files
#resultsFolder = '/Users/adamtrebach/Documents/Research/TPV/Nanowires/evolution_under_node_removal/'
#resultsFolder = '/home/amwt/TPV/2DNanowires/Data/evolution_under_node_removal/bc_paper_node_removal_figure_0013/'
resultsFolder = '/Users/adamtrebach/Documents/Research/TPV/Nanowires/Results/bc_paper_node_removal_figure_0013_v3/'
...
percMultiplesToSample = [1.2, 1.5, 1.8, 2.1]
...
for percMultiple in percMultiplesToSample:
	if shouldEvolveNetworks:
		for trialNum in range(numSamples):
			...
			pickleName = resultsFolder + '_'.join([str(width) + 'x' + str(height),
			...
			if shouldCalculateInitialCentralities:
				...
				c = nwm.percolation_centrality(g, attribute = 'voltage', weight = 'resistance', downPotentialOnly = False)
			...
			removalMethodsToCalculate = set(removalMethodsList) - set(removalMethodsToSkip)
			removalMethodsToCalculate = list(removalMethodsToCalculate)
			...
			for removalMethod in removalMethodsToCalculate:
				...
				nodesToRemovePerStep = 100 if 'low' in removalMethod  else 20
...
#						h.remove_node(node)
	...
	if shouldMakePlot:
#		plotScriptFileName = '/Users/adamtrebach/Documents/Research/TPV/Nanowires/evolution_under_node_removal/evolution_under_node_removal_multiplot_' + str(width) + 'x' + str(height) + 'percMult' + str(percMultiple) + 'plot_script.txt'
		plotScriptFileName = resultsFolder  + str(width) + 'x' + str(height) + 'percMult' + str(percMultiple) + 'plot_script.txt'
		...
		for trialNumber in range(numSamples):
			...
			for n, removalMethod in enumerate(removalMethodsToCalculate):
				fileName = resultsFolder + str(width) + 'x' + str(height) + '_percMult' + str(percMultiple) + '_nwLen' + str(nwLength) + '_Rc' + str(rcMean) + '_trial' + str(int(trialNumber)) + removalMethod + '_data'
				if shouldNormalizeResistances:
					nData = np.loadtxt(fileName)
					nData[:,1] = nData[:,1] / nData[0,1]
					fileName += '_normalized'
					np.savetxt(fileName, nData)
				command = ('plot \"' if n == 0 else '\"') + fileName + '\" using 1:2 with lines title \"' + removalMethod + ' nodes removed first\" lc \"' + colorsDict[removalMethod] + '\"'
...
#				command = '\"' + minMarkerFileName + '\" using 1:2 with points pointsize 10 pointtype 2 lc \"' + colorsDict[removalMethod] + '\" notitle'
				command = '\"' + minMarkerFileName + '\" using 1:2 with lines lw 0.7 lc \"' + colorsDict[removalMethod] + '\" dt \"   .   \" notitle'
				subplotCommandsList.append(command)
...
if shouldMakeAverageFailurePlot:
	...
	# making container for the high density data to be compared to predictions from forro
...
#					if removalMethod == 'lowBC':
	...
	# making data to compare to forro
	for n, removalMethod in enumerate(removalMethodsToCalculate):
...
#					yrange = '5:' + str(maxResistance + 1),
	...
	for removalMethod in removalMethodsToCalculate:
		...
		fileName = resultsFolder + str(width) + 'x' + str(height) + '_nwLen' + str(nwLength) + '_Rc' + str(rcMean) + '_removalMethod=' + removalMethod + '.txt'
		removalMethodFileNameDict[removalMethod] = fileName
		dfToPrint.to_csv(fileName, index = False, sep = ' ')
	
	#making plot script

	plotScriptFileName = resultsFolder + str(width) + 'x' + str(height) + '_nwLen' + str(nwLength) + '_Rc' + str(rcMean) + 'comparing_failure_density_plot_script.txt'
